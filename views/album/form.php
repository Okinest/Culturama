<?php
$pageTitle = "Culturama - Ajouter un album";
...
<div class="bg-white p-8 rounded-lg shadow-md w-full max-w-md mx-auto mt-10">
    <h1 class="text-2xl font-bold mb-6 text-center text-gray-800">Ajouter un album</h1>
    <form method="post" action="/album/add" enctype="multipart/form-data" class="space-y-4">
        <div>
            <label for="albumImage" class="block text-gray-700 font-medium mb-1">Image de l'album :</label>
            <input type="file" id="albumImage" name="albumImage" accept="image/*" class="w-full px-3 py-2 border border-gray-300 rounded">
        </div>
        <div>
        ...
        <div class="flex items-center gap-2">
            ...
            <input type="checkbox" id="isAvailable" name="isAvailable" class="mr-2 align-middle"
                <?= isset($_POST['isAvailable']) ? 'checked' : '' ?>>
        </div>
        <div class="text-center">
            <input type="submit" value="Ajouter l'album" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 cursor-pointer font-semibold">
        </div>
